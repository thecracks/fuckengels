...
<table class = "table table-bordered" id="tablenotasxcursobim_rn_do">
    <tr>
    ...
    <tr>
        ...
        foreach ($arrayCabecera as $value) {
            if ($value !== 'P' && $value !== 'AN' && $value !== 'id') {
                ?>
                <th  rowspan = "2" style="layout-flow:'vertical-ideographic'; filter='progid:DXImageTransform.Microsoft.BasicIma ge(rotation=2)'"><?= $value ?> </th>
                <?php
            }
        }
        ?>
    ...
    for ($idfila = 0; $idfila < $nfilas; $idfila++) {
        ...
        for ($idcolu = 1; $idcolu < $ncolumnas; $idcolu++) {
            ...
            if ($arrayCabecera[$idcolu] !== "P" and $arrayCabecera[$idcolu] !== "AN") {
                ?>
                <td ><?= $arrayDataNuevo[$idfila][$idcolu] ?>" ></td>
